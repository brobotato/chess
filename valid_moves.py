def valid_moves(piece, current_board):
    ...
    if 'king' in piece[2]:
        ...
        moves += valid_moves([piece[0], piece[1], 'rook'], current_board) + valid_moves([piece[0], piece[1], 'bishop'],
                                                                                        current_board)
    elif 'rook' in piece[2]:
        moves += [[piece[0], a] for a in range(0, 8)] + [[b, piece[1]] for b in range(0, 8)]
    elif 'bishop' in piece[2]:
        moves += [[piece[0] + a, piece[1] - a] for a in range(-7, 8)] + [[piece[0] + a, piece[1] + a] for a in
                                                                         range(-7, 8)]
